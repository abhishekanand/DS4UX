...
url_base = 'https://en.wikipedia.org/w/api.php?action=query&prop=revisions&titles=%s&rvlimit=50&rvprop=timestamp|user|userid|comment&continue=&format=json'
...
for page_title in pages:
    ...
    for page_id in response["query"]["pages"].keys():
        ...
        for rev in revisions:
            print(page_title + "\t" + rev["user"] + "\t" + rev["timestamp"] + "\t" + str(rev["userid"]) + "\t" + rev["comment"])
